...
%leaderFrame
%%
name = ['.\Videos\SV03_Pax2_MRS2500_HD'];
ext = '.mp4';
fileOutput = [name ext];
if strcmp(ext,'.avi')
...
for i = 560:2:1200
    ...
    elseif i >= 600
        ...
        hold on;
        if i >= drugApp
            thandle = text(10, 40,'+MRS2500','FontSize',6,'Color','w','FontWeight','bold');
        end
        
        if mod(i,2) == 0 
          timernum = num2str((i-600)/2);
        end
        timer = text(480-10,480-15,[timernum ' s'],'FontSize',6,'color','w','FontWeight','bold','HorizontalAlignment','right');
    ...
    if i >= 560
        thandle0 = text(10,15,'P0\it Pax2-Cre;GCaMP3','FontSize',6,'color','w','FontWeight','bold');
    end
...
name = ['.\Videos\SV02_ISCIHCanalysis_HD'];
ext = '.mp4';
fileOutput = [name ext];
if strcmp(ext,'.avi')
    v = VideoWriter(fileOutput,'Uncompressed AVI');
     ...
     if i <= 0
        ...
        indx_green = find(labels(i,:)>=1);
            ...
            if ~isempty(indx_green)
                for j = 1:size(indx_green,2)
                    temp = indx_green(j);
                    drawellipse('Position',IHClocation(temp,:),'SemiAxes',[1 1],'Color','w','LineWidth',1,'InteractionsAllowed','none');
                    %drawpoint(gca, 'Position',IHClocation(temp,:),'Color','w','MarkerSize',1,'LineWidth',0);
                end
            end          
...
%%
name = ['.\Videos\SV04_ApexvsBase_HD'];
ext = '.mp4';
fileOutput = [name ext];
if strcmp(ext,'.avi')
    v = VideoWriter(fileOutput,'Uncompressed AVI');
...
for i = -20:1:600
    if i < 1
        ...
        timer = text(480-5,240-15,[timernum ' s'],'FontSize',6,'color','w','FontWeight','bold','HorizontalAlignment','right');

        if mod(i,100) == 0
            disp(i);
        end
    end
    thandle = text(10, 15,'P0 Apex','FontSize',6,'Color','w','FontWeight','bold');
    thandle2 = text(240+10,15,'P0 Base','FontSize',6,'Color','w','FontWeight','bold');
    writeVideo(v,getframe); 
end
